...
[![VirusTotal](https://img.shields.io/badge/VirusTotal-отчёт_v1.0.5-394EFF?logo=virustotal&logoColor=white)](https://www.virustotal.com/gui/file/a239a4eb5ab195f60166e50f7256654b1a7527d7534e5bdf0d584d2b9de3b8a9)

| Версия | Файл | SHA-256 | VirusTotal |
| --- | --- | --- | --- |
| v1.0.5 | `zapret2-youtube-discord-v1.0.5.zip` | `a239a4eb5ab195f60166e50f7256654b1a7527d7534e5bdf0d584d2b9de3b8a9` | [Открыть отчёт](https://www.virustotal.com/gui/file/a239a4eb5ab195f60166e50f7256654b1a7527d7534e5bdf0d584d2b9de3b8a9) |
...
Проект также опубликован в [Zapret Hub Marketplace от goshkow](https://goshkow.com/zapret-hub/marketplace/projects/https_github_com_klondike0x_zapret2_youtube_discor). Карточка имеет статус `published` и прошла модерацию площадки. Это дополнительная независимая проверка публикации, но не замена SHA-256, GPG-подписи и отчёта VirusTotal конкретного релиза.
...
- запустить последовательную проверку стратегий.

Пункт `Run strategy tests` проверяет синтаксис каждого профиля через настоящий `winws2.exe`, по очереди запускает стратегии и проверяет доступность Discord и YouTube через HTTP, TLS 1.2 и TLS 1.3. В консоли можно добавить URL или ping-адрес на один запуск, сохранить пользовательские цели в `tools/targets.txt`, удалить их или восстановить стандартный набор. Ввод проверяется и не выполняется как команда. Результаты сохраняются в `tools/test results/`. Перед проверкой остановите службу и закройте вручную запущенные профили. Итог зависит от сети и провайдера, поэтому тест показывает сравнительный результат, а не гарантированно лучшую стратегию.
...
Проверьте, не работает ли уже `winws2.exe` или служба `winws2` с отображаемым именем `zapret2 YouTube Discord`. Закройте окно активного профиля либо остановите процесс через пункт 6 в `service.bat`.
