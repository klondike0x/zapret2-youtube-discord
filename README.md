...
- удалить службу;
- запустить диагностику движка, BFE, WinDivert и известных конфликтов;
- прогнать Flowseal-подобный тест стратегий с HTTP/TLS 1.2/TLS 1.3 и ping-проверками;
- принудительно остановить вручную запущенный `winws2.exe`.

Служба регистрирует непосредственно `bin\winws2.exe`, а не PowerShell-обёртку. При установке выбранный профиль копируется в `tools/service-active.txt` с абсолютным `--chdir`, чтобы Lua, списки и payload-файлы разрешались из корня проекта. Runtime-файл создаётся локально и не входит в Git или релизный архив.

Пункт **Run Strategy Tests** позволяет проверить все `profiles/general*.txt` либо выбранный диапазон. Перед сетевыми запросами каждый профиль проходит настоящий parser dry-run и затем запускается с исходными capture-фильтрами, включая HTTPS/443. Чтобы результаты не смешивались с другой стратегией, тест отказывается продолжать при уже запущенном `winws2.exe`; чужие процессы он не завершает. Результаты сохраняются локально в `tools/test results/`.

Рейтинг показывает только количество успешных транспортных подключений `curl` в текущей сети и в момент запуска. HTTP-ошибка или страница-заглушка тоже может дать успешный код `curl`, поэтому результат не доказывает обход блокировки, не определяет универсально лучшую стратегию и не заменяет проверку YouTube, Discord Voice и других реальных сценариев.
...
Имя `winws2` обязательно: оно зафиксировано внутри официального Windows-бинарника Zapret2 при регистрации обработчика SCM. Отображаемое имя службы остаётся `zapret2 YouTube Discord`.
...
`validate_service_manager.py` проверяет Flowseal-подобное меню службы, прямую регистрацию `winws2.exe`, безопасную подготовку service-профиля и self-test инструмента проверки стратегий.

`validate_readme_sync.py` проверяет наличие обеих языковых версий, одинаковую структуру разделов и ключевые общие факты в `README.md` и `README.en.md`.
...
Проверьте, не работает ли уже `winws2.exe` или служба `winws2`. Закройте окно активного профиля либо остановите ручной процесс этого комплекта через пункт 8 в `service.bat`. Пункт 8 не завершает службу и экземпляры `winws2.exe` из других каталогов.
